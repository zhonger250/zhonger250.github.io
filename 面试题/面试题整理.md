...
了优化,将数据结构优化为数组+链表+红黑树。如果链表长度大于阈值(8) 数组长度超过64,则进行树进化(将链表转为红黑树).
```

### 6 HashMap添加元素的过程 

```
1.首先判断数组是否为空, 如果数组为空, 则对数组进行初始化
2.如果数组不为空, 则通过数组长度-1与新增元素的key的hash值进行位运算, 得到数组下标, 通过下标获得数组元素
3.判断获得的数组元素是否为空, 如果为空,则使用新增键值创建节点, 将节点放在数组的此位置上.
4.如果数组元素不为空, 判断当前数组是否正在扩容, 如果正在扩容, 帮助其扩容.
5.如果该位置是链表, 就遍历该链表, 如果链表中不包含要插入的键值对节点,则该节点放在链表的最后. 如果要插入键值对存在, 则用新的值替换老的值. 如果链表长度大于阈值,  数组长度超过64, 则进行树进化(将链表转化为红黑树).
6.如果不是链表的则进行红黑树操作, 将值放入到红黑树中
...
```
1.判断key和value是否为null, 如果key或者key为null, 直接抛出异常
2.如果不为null. .判断hashMap中的数值是否为空, 如果为空,进行初始化
3.如果数组不为空, 通过数组长度-1与新增元素的key的Hash值进行位运算, 得到数组的下标, 通过下标获得对应的数组元素, 
4.判断获得的数组元素是否为空, 如果为空, 则使用新增键值创建节点,将节点放到数组的该位置上(使用CAS操作)
5.如果数组元素不为空, 判断当前数组是否正在扩容, 如果正在扩容, 帮助其扩容. 否则将数组元素所在链表或红黑树上锁.
6.如果该位置是链表, 就遍历该链表, 如果链表中不包含要插入的键值对节点,则该节点放在链表的最后. 如果要插入键值对存在,
则用新的值替换老的值. 如果链表长度大于阈值,数组长度超过64, 则进行树进化(将链表转化为红黑树).
7.如果不是链表的则进行红黑树操作, 将值放入到红黑树中
